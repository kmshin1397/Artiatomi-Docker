function iterative_alignment(opts)
    ...
    for i = 1:opts.iters
        ...
        artia.mpi.run(opts.EmSART, opts.nodes, oss10CfgFile, 'runRemote', opts.remote, 'remoteHost', opts.host, 'remotePort', opts.port, 'suppressOutput', false)
        artia.mpi.run(opts.EmSART, opts.nodes, oss1CfgFile, 'runRemote', opts.remote, 'remoteHost', opts.host, 'remotePort', opts.port, 'suppressOutput', false)
        artia.mpi.run(opts.EmSART, opts.nodes, oss1FilteredCfgFile, 'runRemote', opts.remote, 'remoteHost', opts.host, 'remotePort', opts.port, 'suppressOutput', false)
...
%         artia.mpi.run(opts.STA, opts.nodes, avgCfgFile, 'runRemote', opts.remote, 'remoteHost', opts.host, 'remotePort', opts.port, 'suppressOutput', false, 'execDir', opts.STA_dir)
%         
        % Do the refinement %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        disp('Doing the refinement ...')
        artia.mpi.run(opts.EmSARTRefine, opts.nodes, refCfgFile, 'runRemote', opts.remote, 'remoteHost', opts.host, 'remotePort', opts.port, 'suppressOutput', false);
        ...
        for j = 1:size(new_motl, 2)
            phi = motive(17, j); 
            psi = motive(18, j); 
            theta = motive(19, j);

            new_motl(17:19, j) = [phi, psi, theta];
        end    
